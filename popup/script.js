function addEntries() {
  ...
  browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
    ...
    browser.scripting.executeScript({
      target: { tabId: tab.id },
      func: autoClickGreaterThan,
      args: [parseInt(dollars, 10)],
    });
  });
}
...
function autoClickGreaterThan(dollars) {
  ...
  const rowsWithValues = [...rows].map((row) => ({
    row,
    value: parseInt(row.innerText.match(/\$(\d+)/)[1], 10),
  }));
  const desiredRows = rowsWithValues.filter(({ value }) => value >= dollars);
  const buttons = desiredRows.map(({ row }) =>
    row.firstChild.querySelector('[title="1-click add"]'),
  );
